...
> **Implementation Status: COMPLETE** — 46 source files, 11 test classes, 46 tests passing, `mvn verify` green.
> 8 strategies (4 Prowide + 4 Velocity), Spring Batch pipeline, REST API, domain + persistence layers, JaCoCo check passes.
> Vanilla HTML/CSS/JS frontend with 6 views (Dashboard, Generate, Runner, History, Benchmarks, Diagrams) in `src/main/resources/static/`.
> 14 architecture diagrams in `docs/diagrams/` (.mmd + .puml). 6 cross-platform run/kill scripts (sh, ps1, bat).
> **Single-profile app** — no Maven profiles, no Spring profiles. `mvn clean install` and `mvn spring-boot:run` are the only commands needed.
...
# Run (Swagger at /swagger-ui.html, H2 console at /h2-console)
mvn spring-boot:run
...
# Makefile shortcuts
make build | make run | make test | make test-unit | make clean | make kill | make lint | make docs | make help
```
...
| Method | Path | Purpose |
|--------|------|---------|
| `POST` | `/api/random-statements` | Generate and persist random `BankStatement` data; `?loadProfile=LOW\|MEDIUM\|HIGH` |
...
Vanilla HTML/CSS/JavaScript — no npm, no build step.
Source served directly from `src/main/resources/static/`:

```
static/
  index.html        — SPA shell, sidebar nav, header, footer
  css/style.css     — all styling (CSS variables for light/dark theme)
  js/app.js         — all logic: routing, API calls, view rendering
```

CDN dependencies loaded at runtime (no local install needed):
- **Chart.js 4.4** — bar and line charts in the Charts view
- **Mermaid 11** — architecture diagrams in the Diagrams view

Views: Dashboard · Generate Statements · Conversion Runner (incl. Run All 8 Combinations) · History · Benchmarks · Diagrams

The SPA uses hash-based routing (`#dashboard`, `#generate`, etc.). Spring Boot's `WebConfig` serves `index.html` as the fallback for any unrecognised path, though hash routing doesn't require server-side fallback.
...
Also rendered live in the **Diagrams** view of the SPA (via Mermaid.js CDN).
...
No profile flags — `--prod` and `-Prod` options have been removed; the app runs in a single configuration.
...
46 tests, 0 failures — `mvn verify` green, JaCoCo ≥40% instruction coverage enforced.
